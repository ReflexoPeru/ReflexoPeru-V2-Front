...
const ModeloTable = ({ columns, data, customActions }) => {
    ...
    const columnaArreglada = columns.map((col, index, arr) => {
        ...
        const FechaCitaColumn = col.key === 'fechaCita' || col.dataIndex === 'fechaCita';
        const SalaColumn = col.key === 'sala' || col.dataIndex === 'sala';
        const TicketColumn = col.key === 'ticket' || col.dataIndex === 'ticket';
        const MetodoColumn = col.key ==='methodPago' || col.dataIndex === 'methodPago';
    ...
    const paginationData = data.slice((currentPage - 1) * pageSize, currentPage * pageSize);
            ...
            >
                <div style={{overflow: 'hidden' }}>
                    <Table
                        ...
                        rowClassName={(__, index) => (index % 2 === 0 ? estilos.zebraRow : '')}
                    />
                </div>
